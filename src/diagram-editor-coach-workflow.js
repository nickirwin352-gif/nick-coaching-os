...
function addStyles() {
  ...
  style.textContent = `
    ...
    #dsSessionDiagramHost .dsMovementHit{
      stroke-width:38!important;stroke-linecap:round!important;stroke-linejoin:round!important;
      pointer-events:stroke!important;touch-action:none!important;cursor:grab!important;
    }
    ...
    #dsSessionDiagramHost .dsMovementVisible.dsMovementHover{filter:drop-shadow(0 0 5px rgba(56,189,248,.95))}
    ...
    #${BAR_ID}{flex:0 0 auto;display:flex;align-items:center;gap:6px;flex-wrap:wrap;padding:7px 9px;border-bottom:1px solid var(--border);background:#09111e}
    ...
    #${PRESET_ID}{flex:0 0 auto;padding:7px 9px;border-bottom:1px solid var(--border);background:linear-gradient(180deg,rgba(15,46,34,.55),rgba(9,17,30,.98))}
    ...
    .coachDragSelectBox{position:fixed;z-index:50000;pointer-events:none;border:2px solid var(--sky);background:rgba(56,189,248,.14);border-radius:6px;box-shadow:0 0 0 1px rgba(56,189,248,.18),0 10px 30px rgba(0,0,0,.18)}
    #dsPitch.coachPasteArmed{cursor:copy!important}
    #dsPitch.coachPasteArmed *{cursor:copy!important}
    @media(max-width:700px){
      #${BAR_ID}{gap:4px;padding:6px}#${BAR_ID} button{padding:8px 8px;font-size:10px}#${BAR_ID} .coachWorkflowHint{width:100%;margin-left:0}
      #${PRESET_ID}{padding:6px}#${PRESET_ID} .coachPresetHead span{display:none}
    }
  `;
...
function addPlayer(x, y, color, label, role = '') { objects().push(newObject('player', { x, y, color, label, role })); }
...
function addMovement(type, points, extras = {}) { objects().push({ id:uid('movement'), type:'movement', movementType:type, points:clone(points), layer:'movement', z:12, ...extras }); }
...
function presetPossessionBox(d) {
  ...
  addMiniGoal(x-30,y+h*.24,90); addMiniGoal(x+w+4,y+h*.62,90); addBall(x+38,y+h-54);
}

function presetBuildOut(d) {
  const L=d.w*.06,R=d.w*.47;
  addGoal(L-28,d.h*.44,90); addPlayer(L+28,d.h*.48,'green','GK','Goalkeeper');
  addPlayer(L+105,d.h*.27,'blue','4','CB'); addPlayer(L+105,d.h*.68,'blue','5','CB');
  addPlayer(L+205,d.h*.14,'blue','2','FB'); addPlayer(L+205,d.h*.81,'blue','3','FB'); addPlayer(L+225,d.h*.48,'blue','6','6');
  addPlayer(R-70,d.h*.34,'orange','9','Press'); addPlayer(R-70,d.h*.62,'orange','10','Press'); addPlayer(R-12,d.h*.48,'orange','8','Screen 6');
  addMiniGoal(R+18,d.h*.18,90); addMiniGoal(R+18,d.h*.76,90); addBall(L+58,d.h*.51);
...
function presetPress442(d) {
  ...
  addPlayer(x0+85,d.h*.18,'blue','2'); addPlayer(x0+85,d.h*.38,'blue','4'); addPlayer(x0+85,d.h*.60,'blue','5'); addPlayer(x0+85,d.h*.80,'blue','3'); addPlayer(x0+185,d.h*.49,'blue','6');
  addPlayer(x1-55,d.h*.39,'orange','9','Press'); addPlayer(x1-55,d.h*.60,'orange','10','Protect'); addPlayer(x1+20,d.h*.18,'orange','11','Inside-out'); addPlayer(x1+20,d.h*.81,'orange','7','Inside-out'); addPlayer(x1+40,d.h*.38,'orange','8'); addPlayer(x1+40,d.h*.62,'orange','6');
...
function presetWideOverload(d) {
  ...
  addMiniGoal(x+w-36,y+18,90); addMiniGoal(x+w-36,y+h-42,90); addBall(x+72,y+h*.20);
  addMovement('run',[{x:x+76,y:y+h*.66},{x:x+205,y:y+h*.72}]);
  addMovement('pass',[{x:x+78,y:y+h*.22},{x:x+170,y:y+h*.37}]);
}

function presetFinishing(d) {
  const x=d.w*.08,R=d.w*.47;
  addGoal(R,d.h*.40,90); addPlayer(R-48,d.h*.48,'green','GK');
  addPlayer(x+70,d.h*.49,'blue','10'); addPlayer(x+175,d.h*.50,'blue','9'); addPlayer(x+105,d.h*.17,'blue','11'); addPlayer(x+105,d.h*.80,'blue','7');
  addPlayer(x+220,d.h*.35,'orange','CB1'); addPlayer(x+220,d.h*.63,'orange','CB2'); addBall(x+86,d.h*.52); addCone(x+25,d.h*.35); addCone(x+25,d.h*.67);
...
function presetRondoGoals(d) {
  ...
  addMiniGoal(x+w+8,y+h*.20,90); addMiniGoal(x+w+8,y+h*.68,90); addBall(x+w*.17,y+h*.20);
}
...
function createWorkflowBar() {
  ...
  if (!bar) {
    ...
    bar.innerHTML=`<button type="button" class="coachWorkflowPrimary" data-coach-copy>⧉ Copy Selection</button><button type="button" class="coachWorkflowPaste" data-coach-paste>⌖ Paste Again</button><button type="button" data-coach-mirror="x">↔ Mirror Side</button><button type="button" data-coach-mirror="y">↕ Mirror Half</button><button type="button" data-coach-progression>＋ Progression</button><button type="button" data-coach-delete>Delete Selected</button><span class="coachWorkflowHint">Drag a box on empty pitch to select a group</span>`;
...
function pasteAt(point) {
  ...
  dx=Math.max(-b.left,Math.min(d.w-b.right,dx)); dy=Math.max(-b.top,Math.min(d.h-b.bottom,dy));
  try { if(typeof dsPushHistory==='function') dsPushHistory(); } catch(_){}
...
function observePitch() {
  ...
  pitchObserver?.disconnect(); pitchObserver=new MutationObserver(()=>{bindArrowHover();renderSelectionState();});
  pitchObserver.observe(pitch,{childList:true,subtree:true}); bindArrowHover();
}
...
function installPointerWorkflow() {
  ...
  document.addEventListener('pointercancel',event=>{if(!dragSelect||event.pointerId!==dragSelect.pointerId)return;dragSelect.box?.remove();dragSelect=null;},true);
}
...
function install() {
  ...
  setTimeout(()=>{observeStudio();ensureUi();},250); setTimeout(()=>{observeStudio();ensureUi();},900);
}
