...
function appDb() {
  try { return db; } catch (_) { return window.db; }
}
...
function addStyles() {
  ...
  style.textContent = `
    ...
    @media(max-width:720px){
      #sessionsLibraryView{padding:14px}
      .sessionLibraryToolbar{grid-template-columns:1fr}
      .sessionLibraryGrid{grid-template-columns:1fr}
      .diagramOverviewGrid{grid-template-columns:1fr}
      .diagramOverviewOverlay,.sessionDetailOverlay{padding-left:10px;padding-right:10px}
    }
  `;
...
function installFastSessionSave() {
  ...
  const fastSave = function(mode = 'new') {
    try {
      ...
      const base = typeof currentPlannerSession === 'function'
        ...
        : {
            ...
            links: links.value,
            cues: cues.value,
            drills: [...plannerDrills],
            diagramOverrides: typeof copyPlannerDiagramOverrides === 'function' ? copyPlannerDiagramOverrides() : [],
...
function plannerAsSession() {
  try { return currentPlannerSession(); } catch (_) {
    return { drills: [...plannerDrills], diagramOverrides: typeof copyPlannerDiagramOverrides === 'function' ? copyPlannerDiagramOverrides() : [] };
  }
}
...
export function openAllSessionDiagrams(sessionOrIndex = null, title = '') {
  ...
  overlay.innerHTML = `<div class="diagramOverviewShell">
    <div class="diagramOverviewHead"><div><h2>${esc(title || session.theme || 'All Session Diagrams')}</h2><div class="small">${esc(session.date || '')}${session.team ? ` · ${esc(session.team)}` : ''} · ${items.length} practices</div></div><button type="button" id="closeDiagramOverview">Close</button></div>
    <div class="diagramOverviewGrid">${items.map((item, i) => `<article class="diagramOverviewCard"><h3>${i + 1}. ${esc(item.stage ? `${item.stage} · ` : '')}${esc(item.name)}</h3>${item.time ? `<div class="small" style="margin-bottom:7px">${esc(item.time)}</div>` : ''}<div id="diagram-overview-${i}"></div></article>`).join('')}</div>
  </div>`;
  ...
  items.forEach((item, i) => setTimeout(() => {
    try { drawMini(`diagram-overview-${i}`, item.diagram, item.pitchMode); } catch (error) { console.error('Diagram overview render failed', error); }
  }, 0));
}
...
function renderSessionLibrary() {
  ...
  if (!root) return;
  const data = appDb();
  const search = document.getElementById('sessionLibrarySearch')?.value || '';
  const teamFilter = document.getElementById('sessionLibraryTeam')?.value || '';
  const themeFilter = document.getElementById('sessionLibraryTheme')?.value || '';
  const sessions = filterSessions(data.sessions || [], { search, team: teamFilter, theme: themeFilter }, practiceById);
  ...
  if (!sessions.length) {
    root.innerHTML = '<div class="notice">No saved sessions match those filters.</div>';
    return;
  }
  root.innerHTML = `<div class="sessionLibraryGrid">${sessions.map(session => {
    ...
    return `<article class="sessionLibraryCard">
      <div class="small">${esc(session.date || 'No date')}${session.team ? ` · ${esc(session.team)}` : ''}</div>
      <h3>${esc(session.theme || 'Session')}</h3>
      <div class="sessionLibraryMeta"><span class="pill">${drills.length} ${drills.length === 1 ? 'practice' : 'practices'}</span>${session.rating ? `<span class="pill">${'★'.repeat(Number(session.rating) || 0)}</span>` : ''}${session.review ? '<span class="pill">Reviewed</span>' : ''}</div>
      <div class="sessionLibraryObjective"><b>Objective:</b> ${esc(session.objective || '—')}</div>
      <div class="sessionLibraryActions">
        <button type="button" data-session-action="view" data-index="${index}">View Session</button>
        <button type="button" data-session-action="diagrams" data-index="${index}">🗺 All Diagrams</button>
        <button type="button" data-session-action="sideline" data-index="${index}">▶ Sideline</button>
        <button type="button" data-session-action="edit" data-index="${index}">Edit</button>
      </div>
    </article>`;
  }).join('')}</div>`;
}
...
function installSessionLibrary() {
  ...
  button.id = 'sessionsLibraryTab';
  button.type = 'button';
  button.className = 'tab';
  button.textContent = 'Sessions';
  const archiveTab = nav.querySelector('[data-tab="archive"]');
  if (archiveTab) nav.insertBefore(button, archiveTab);
  else nav.appendChild(button);

  const view = document.createElement('section');
  view.id = 'sessionsLibraryView';
  view.className = 'view hidden';
  view.innerHTML = `<div class="card"><h2>Saved Sessions</h2><p class="small">Every saved session in one place. Search it, open it, start Sideline Mode, or view every diagram in one tap.</p>
    <div class="sessionLibraryToolbar">
      <input id="sessionLibrarySearch" type="search" placeholder="Search objective, practice, theme…" aria-label="Search saved sessions">
      <select id="sessionLibraryTeam" aria-label="Filter sessions by team"><option value="">All teams</option></select>
      <select id="sessionLibraryTheme" aria-label="Filter sessions by theme"><option value="">All themes</option></select>
    </div>
    <div class="sessionLibrarySummary"><b id="sessionLibraryCount">0 sessions</b><button type="button" id="sessionLibraryClearFilters">Clear filters</button></div>
    <div id="sessionLibraryResults"></div>
  </div>`;
  ...
  document.getElementById('sessionLibraryClearFilters')?.addEventListener('click', () => {
    document.getElementById('sessionLibrarySearch').value = '';
    document.getElementById('sessionLibraryTeam').value = '';
    document.getElementById('sessionLibraryTheme').value = '';
    renderSessionLibrary();
  });
  document.getElementById('sessionLibraryResults')?.addEventListener('click', event => {
    const control = event.target.closest('[data-session-action]');
    if (!control) return;
    const index = Number(control.dataset.index);
    switch (control.dataset.sessionAction) {
      case 'view': openSessionDetail(index); break;
      case 'diagrams': openAllSessionDiagrams(index); break;
      case 'sideline': try { openGrassView(index); } catch (_) {} break;
      case 'edit': try { loadSessionToPlanner(index); } catch (_) {} break;
    }
  });
}
...
function injectPlannerDiagramButton() {
  ...
  let hasDrills = false;
  try { hasDrills = Array.isArray(plannerDrills) && plannerDrills.length > 0; } catch (_) {}
  if (!hasDrills) return;
  const button = document.createElement('button');
  button.id = 'plannerDiagramOverviewButton';
  button.className = 'plannerDiagramOverviewButton';
  button.type = 'button';
  button.textContent = '🗺 View All Diagrams';
...
function wrapPreview() {
  let original;
  try { original = renderPreview; } catch (_) { original = window.renderPreview; }
  if (!original || original.__diagramOverviewWrapped) return;
  const wrapped = function(...args) {
    const result = original.apply(this, args);
    injectPlannerDiagramButton();
    return result;
  };
...
function wrapSessionCards() {
  ...
  const wrapped = function(session, index, ...rest) {
    ...
    if (card && !card.querySelector('[data-all-diagrams]')) {
      ...
      const button = document.createElement('button');
      button.type = 'button';
      button.dataset.allDiagrams = '1';
      button.textContent = '🗺 View All Diagrams';
      button.addEventListener('click', event => { event.stopPropagation(); openAllSessionDiagrams(index); });
...
function install() {
  addStyles();
  installFastSessionSave();
  installSessionLibrary();
  wrapPreview();
  wrapSessionCards();
}
