...
export function latestPracticeReview(sessions = [], practiceId = '') {
  const ordered = [...sessions].sort((a, b) => {
    const aKey = String(a?.review?.reviewedAt || a?.date || '');
    const bKey = String(b?.review?.reviewedAt || b?.date || '');
    return bKey.localeCompare(aKey);
  });
  for (const session of ordered) {
    const practice = (session?.review?.practices || []).find(item => item?.practiceId === practiceId);
    if (!practice) continue;
    return {
      session,
      review: practice,
      score: practiceScoreOutOfTen(practice, session),
      reasoning: String(practice.reasoning || practice.note || '').trim()
    };
  }
  return null;
}
...
function applyQuality(element, score) {
  ...
  if (value > 0) {
    element.style.setProperty('--quality-hue', String(trafficHue(value)));
    element.style.setProperty('--quality-score', String(value));
  } else {
    element.style.removeProperty('--quality-hue');
    element.style.removeProperty('--quality-score');
  }
}
...
function scoreBadge(score, label = '') {
  const value = Number(score) || 0;
  if (!value) return '';
  return `<span class="qualityScoreBadge" style="--quality-hue:${trafficHue(value)}">${value}/10${label ? ` · ${esc(label)}` : ''}</span>`;
}
...
function addStyles() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    /* Session subtitle / format */
    #${SUBTITLE_FIELD_ID}{margin-top:10px}
    #${SUBTITLE_FIELD_ID} label{margin-top:0}
    #${SUBTITLE_FIELD_ID} .sessionSubtitleHint{font-size:10.5px;color:var(--text-dim);margin-top:5px;line-height:1.35}
    .sessionSubtitleText{font-size:12px;font-weight:800;color:#bae6fd;margin:-2px 0 7px;letter-spacing:.01em}
    .sessionPreviewSubtitle{margin:2px 0 10px;color:#bae6fd;font-weight:800;font-size:13px}

    /* Traffic-light session quality. 1 = dark red, 5 = yellow/orange, 10 = dark green. */
    ...
    .qualityScoreBadge{display:inline-flex;align-items:center;justify-content:center;gap:4px;padding:4px 8px;border-radius:999px;font-size:11px;font-weight:900;white-space:nowrap;color:#fff;background:hsl(var(--quality-hue) 61% 31%);border:1px solid hsl(var(--quality-hue) 72% 48%)}
    ...
    .practiceReviewMemory{margin-top:7px}
    .reviewMemoryReason{display:block;margin-top:3px;color:#dce7f5}

    /* Review: rating + reasoning sit together, while secondary dropdowns remain compact. */
    ...
    #postSessionReviewOverlay .reviewReasoningField{display:block!important;grid-column:2;grid-row:1 / span 3;align-self:stretch}
    #postSessionReviewOverlay .reviewReasoningField.reviewNoteHidden{display:block!important}
    ...
    #postSessionReviewOverlay .reasoningHint{font-size:10px;color:var(--text-dim);margin-top:4px;line-height:1.35}

    /* Calendar traffic-light view with score under the session title. */
    ...
    .calSessionDot .calSessionTitle{display:block;font-size:10px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .calSessionDot .calSessionScore{display:block;margin-top:2px;font-size:10px;font-weight:950;color:#fff}
    .calSessionDot .calSessionSubtitle{display:block;margin-top:1px;font-size:8.5px;color:rgba(255,255,255,.78);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    ...
    @media(max-width:760px){
      ...
      .calSessionDot.sessionQualityDot{padding:3px 4px!important}.calSessionDot .calSessionTitle,.calSessionDot .calSessionScore{font-size:9px}.calSessionDot .calSessionSubtitle{font-size:8px}
    }
  `;
...
function subtitleInput() {
  return document.getElementById('sessionSubtitle');
}
...
function ensureSubtitleField() {
  ...
  wrap.innerHTML = `
    <label for="sessionSubtitle">Session Subtitle / Format</label>
    <input id="sessionSubtitle" list="sessionSubtitleSuggestions" placeholder="e.g. Patterns, SSGs, Phase of Play, Unit Work">
    <datalist id="sessionSubtitleSuggestions"><option value="Patterns"><option value="SSGs"><option value="Phase of Play"><option value="Unit Work"><option value="Rondo"><option value="Opposed Practice"><option value="Unopposed Pattern"><option value="Game Based"></datalist>
    <div class="sessionSubtitleHint">Keep the main theme broad, then use this for the specific type of session you ran.</div>`;
...
function currentSubtitle() {
  return String(subtitleInput()?.value || '').trim();
}
...
function wrapPlannerData() {
  ...
  if (typeof original === 'function' && !original.__sessionSubtitle) {
    const wrapped = function(...args) {
      ...
      return { ...result, subtitle: currentSubtitle() };
    };
  ...
  if (typeof loadOriginal === 'function' && !loadOriginal.__sessionSubtitle) {
    const wrappedLoad = function(index, mode = 'edit') {
    ...
    wrappedLoad.__sessionSubtitle = true;
    try { loadSessionToPlanner = wrappedLoad; } catch (_) {}
  ...
  if (typeof resetOriginal === 'function' && !resetOriginal.__sessionSubtitle) {
    ...
    wrappedReset.__sessionSubtitle = true;
    try { resetSessionPlanner = wrappedReset; } catch (_) {}
    window.resetSessionPlanner = wrappedReset;
  }
  ...
  if (typeof draftOriginal === 'function' && !draftOriginal.__sessionSubtitle) {
    const wrappedDraft = function(...args) {
      const result = draftOriginal.apply(this, args);
      ensureSubtitleField();
      if (subtitleInput()) subtitleInput().value = '';
      return result;
    };
    wrappedDraft.__sessionSubtitle = true;
    try { applyDraftDetails = wrappedDraft; } catch (_) {}
    window.applyDraftDetails = wrappedDraft;
  }
}
...
function wrapPreviewAndDock() {
  ...
  if (typeof previewOriginal === 'function' && !previewOriginal.__sessionIntelligence) {
    const wrappedPreview = function(...args) {
      const result = previewOriginal.apply(this, args);
      decoratePreviewSubtitle();
      return result;
    };
    wrappedPreview.__sessionIntelligence = true;
    try { renderPreview = wrappedPreview; } catch (_) {}
  ...
  if (typeof dockOriginal === 'function' && !dockOriginal.__sessionSubtitle) {
    const wrappedDock = function(...args) {
    ...
    wrappedDock.__sessionSubtitle = true;
    try { renderCurrentSessionDock = wrappedDock; } catch (_) {}
    window.renderCurrentSessionDock = wrappedDock;
  }
}
...
function enhanceReasoningFields() {
  ...
  [...overlay.querySelectorAll('.reviewPractice')].forEach((row, index) => {
    const effect = row.querySelector('.practiceEffect')?.closest('div');
    const engagement = row.querySelector('.practiceEngagement')?.closest('div');
    const decision = row.querySelector('.practiceDecision')?.closest('div');
    const textarea = row.querySelector('.practiceNote');
    const reasoning = textarea?.closest('div');
    effect?.classList.add('reviewEffectField');
    engagement?.classList.add('reviewEngagementField');
    decision?.classList.add('reviewDecisionField');
    if (!reasoning || !textarea) return;
    reasoning.classList.remove('reviewNoteHidden');
    reasoning.classList.add('reviewReasoningField');
    textarea.classList.add('practiceReasoning');
    const label = reasoning.querySelector('label');
    ...
    if (!reasoning.querySelector('.reasoningHint')) {
      const hint = document.createElement('div');
      hint.className = 'reasoningHint';
      hint.textContent = 'This is saved with the practice so you can understand the score when you consider using it again.';
      reasoning.appendChild(hint);
    }
  });
}

function selectedChoice(name) {
  return document.querySelector(`[data-choice="${name}"] button.on`)?.dataset.value || '';
}

function summariseReview(review) {
  return [
    review.worked && `Worked: ${review.worked}`,
    review.didntWork && `Didn't: ${review.didntWork}`,
    review.repeat && `Repeat: ${review.repeat}`,
    review.changeNext && `Change next: ${review.changeNext}`
  ].filter(Boolean).join('\n');
}
...
function updateCloudStatusSafe(text) {
  try { if (typeof updateCloudStatus === 'function') updateCloudStatus(text, new Date().toLocaleString('en-GB')); } catch (_) {}
}

function saveLocalSnapshot(data) {
  try { localStorage.setItem('nickCoachOSv3', JSON.stringify(data)); return true; }
  catch (error) { console.error('Local review save failed', error); return false; }
}
...
async function flushReviewCloud() {
  ...
  if (!window.nickCloud || typeof window.nickCloud.save !== 'function') {
    ...
    else updateCloudStatusSafe('Saved locally · cloud sync pending');
    return;
  }
...
function refreshAfterReviewSave() {
  try { if (typeof window.renderSessionLibrary === 'function') window.renderSessionLibrary(); } catch (_) {}
...
function saveReviewInstantly(openDashboard) {
  ...
  const rows = [...document.querySelectorAll('#postSessionReviewOverlay .reviewPractice')];
  const previous = session.review?.practices || [];
  const practices = rows.map((row, index) => {
    const existing = previous[index] || {};
    const reasoning = String(row.querySelector('.practiceReasoning,.practiceNote')?.value || '').trim();
    return {
      ...existing,
      practiceId: row.dataset.practiceId,
      effectiveness: row.querySelector('.practiceEffect')?.value || '',
      engagement: row.querySelector('.practiceEngagement')?.value || '',
      decision: row.querySelector('.practiceDecision')?.value || '',
      reasoning,
      note: reasoning
    };
  });
  const review = {
    ...(session.review || {}),
    scale: 10,
    objectiveOutcome: selectedChoice('objectiveOutcome'),
    rating: document.getElementById('reviewRating')?.value || '',
    worked: String(document.getElementById('reviewWorked')?.value || '').trim(),
    didntWork: String(document.getElementById('reviewDidnt')?.value || '').trim(),
    repeat: String(document.getElementById('reviewRepeat')?.value || '').trim(),
    changeNext: String(document.getElementById('reviewChange')?.value || '').trim(),
    practices,
  ...
  if (saved) {
    queueReviewCloud(data);
    showReviewToast('Review saved · syncing in background');
  } else {
    showReviewToast('Could not save review locally');
  }
  ...
  refreshAfterReviewSave();
  if (openDashboard) {
    document.querySelector('.tab[data-tab="dashboard"]')?.click();
    requestAnimationFrame(() => {
      try { if (typeof renderDashboard === 'function') renderDashboard(); else window.renderDashboard?.(); } catch (_) {}
      try { window.renderCoachingDashboardActions?.(); } catch (_) {}
    });
  }
  return true;
}
...
function wrapReviewOpen() {
  ...
  const wrapped = function(session, index) {
    reviewSessionRef = session || null;
    const result = original.apply(this, arguments);
    requestAnimationFrame(enhanceReasoningFields);
    setTimeout(enhanceReasoningFields, 60);
    return result;
  };
...
function observeReview() {
  ...
  reviewObserver = new MutationObserver(() => {
    if (overlay.classList.contains('open')) requestAnimationFrame(enhanceReasoningFields);
  });
...
function sessionMemoryMarkup(session) {
  ...
  const practiceReasons = (session?.review?.practices || []).map(item => String(item.reasoning || item.note || '').trim()).filter(Boolean);
  const reason = overall || practiceReasons[0] || '';
  if (!score && !reason) return '';
  const label = score ? `${score}/10` : 'Reviewed';
  return `<div class="sessionReviewMemory" style="--quality-hue:${trafficHue(score || 5)}"><b>Review memory · ${label}</b>${reason ? `<span class="reviewMemoryReason">${esc(reason)}</span>` : ''}</div>`;
}
...
function decorateSessionLibrary() {
  ...
  [...root.querySelectorAll('.sessionLibraryCard')].forEach(card => {
    ...
    if (heading && session.subtitle) {
      const subtitle = document.createElement('div');
      subtitle.className = 'sessionSubtitleText';
    ...
    const objective = card.querySelector('.sessionLibraryObjective');
    if (objective) objective.insertAdjacentHTML('afterend', sessionMemoryMarkup(session));
  });
}
...
function decorateRecentSessions() {
  ...
  [...box.querySelectorAll('.recentSessionCard')].forEach((card, index) => {
    ...
    if (actions) actions.insertAdjacentHTML('beforebegin', sessionMemoryMarkup(session));
  });
}
...
function decoratePracticePicker() {
  ...
  [...picker.querySelectorAll('.pitchCard')].forEach(card => {
    const heading = card.querySelector('h3');
    const id = String(heading?.textContent || '').split(' · ')[0].trim();
    if (!id) return;
    ...
    const decision = latest.review?.decision ? ` · ${latest.review.decision}` : '';
    block.innerHTML = `<b>Last review${latest.score ? ` · ${latest.score}/10` : ''}${esc(decision)}</b>${latest.reasoning ? `<span class="reviewMemoryReason">${esc(latest.reasoning)}</span>` : ''}`;
...
function previewData(practice) {
  ...
  if (first?.diagram) return { diagram:first.diagram, pitchMode:first.pitchMode || practice.pitchMode || 'full' };
  return { diagram:practice?.diagram || [], pitchMode:practice?.pitchMode || 'full' };
}
...
function redrawHost(host, practice) {
  ...
  requestAnimationFrame(() => {
    try { if (typeof drawMini === 'function') drawMini(host.id, source.diagram, source.pitchMode); else window.drawMini?.(host.id, source.diagram, source.pitchMode); } catch (_) {}
  });
}
...
function archiveReasoningMarkup(session) {
  ...
  const rows = (session?.review?.practices || []).map((review, index) => {
    ...
    const score = practiceScoreOutOfTen(review, session);
...
function decorateArchiveCard(card, session) {
  ...
  const ids = Array.isArray(session.drills) ? session.drills : [];
  const favouriteHosts = [...card.querySelectorAll('.favGrid > div')];
  favouriteHosts.forEach((wrapper, index) => {
    const host = wrapper.querySelector('div[id]');
    redrawHost(host, effectivePractice(session, index));
  });
  const details = [...card.querySelectorAll('.practiceDetail')];
  details.forEach((detail, index) => redrawHost(detail.firstElementChild, effectivePractice(session, index)));

  card.querySelector('.archivePracticeReasoning')?.remove();
  const actions = card.querySelector('.sessionActions');
  const reasoning = archiveReasoningMarkup(session);
  if (actions && reasoning) actions.insertAdjacentHTML('beforebegin', reasoning);

  const ratingSelect = actions?.querySelector('select');
  if (ratingSelect && ratingSelect.dataset.tenScale !== 'true') {
...
function wrapBuildSessionCard() {
  ...
  const wrapped = function(session, index) {
    ...
    return decorateArchiveCard(card, session);
  };
  wrapped.__sessionIntelligence = true;
...
function decorateCalendar() {
  ...
  let month;
  try { month = archiveMonth; } catch (_) { month = new Date(); }
  ...
  [...calendar.querySelectorAll('.calDay')].forEach(cell => {
    ...
    dots.forEach((dot, index) => {
      ...
      const subtitle = session.subtitle ? `<span class="calSessionSubtitle">${esc(session.subtitle)}</span>` : '';
      dot.innerHTML = `<span class="calSessionTitle">${esc(session.theme || 'Session')}</span>${subtitle}<span class="calSessionScore">${score ? `${score}/10` : 'Not reviewed'}</span>`;
    });
  });
}
...
function wrapArchiveRender() {
  ...
  const wrapped = function(...args) {
    const result = original.apply(this, args);
    requestAnimationFrame(() => { decorateCalendar(); refreshDecorations(); });
    return result;
  };
  wrapped.__sessionIntelligence = true;
...
function wrapRecentAndPicker() {
  ...
  if (typeof recentOriginal === 'function' && !recentOriginal.__sessionIntelligence) {
    const wrappedRecent = function(...args) {
      const result = recentOriginal.apply(this, args);
      requestAnimationFrame(decorateRecentSessions);
      return result;
    };
    wrappedRecent.__sessionIntelligence = true;
    try { renderRecentSessions = wrappedRecent; } catch (_) {}
    window.renderRecentSessions = wrappedRecent;
  }
  ...
  if (typeof pickerOriginal === 'function' && !pickerOriginal.__sessionIntelligence) {
    const wrappedPicker = function(...args) {
      const result = pickerOriginal.apply(this, args);
      requestAnimationFrame(decoratePracticePicker);
      return result;
    };
    wrappedPicker.__sessionIntelligence = true;
    try { renderVisualPicker = wrappedPicker; } catch (_) {}
    window.renderVisualPicker = wrappedPicker;
  }
}

function observeDecoratedAreas() {
  const library = document.getElementById('sessionLibraryResults');
  if (library && !libraryObserver) {
    libraryObserver = new MutationObserver(() => requestAnimationFrame(decorateSessionLibrary));
    libraryObserver.observe(library, { childList:true, subtree:true });
  }
  const recent = document.getElementById('recentSessionList');
  if (recent && !recentObserver) {
    recentObserver = new MutationObserver(() => requestAnimationFrame(decorateRecentSessions));
    recentObserver.observe(recent, { childList:true, subtree:true });
  }
  const picker = document.getElementById('visualPicker');
  if (picker && !pickerObserver) {
    pickerObserver = new MutationObserver(() => requestAnimationFrame(decoratePracticePicker));
    pickerObserver.observe(picker, { childList:true, subtree:true });
  }
  const calendar = document.getElementById('archiveCalendar');
  if (calendar && !archiveObserver) {
    archiveObserver = new MutationObserver(() => requestAnimationFrame(decorateCalendar));
    archiveObserver.observe(calendar, { childList:true, subtree:true });
  }
}

function refreshDecorations() {
  ensureSubtitleField();
  enhanceReasoningFields();
  decorateSessionLibrary();
  decorateRecentSessions();
  decoratePracticePicker();
  decorateCalendar();
  decoratePreviewSubtitle();
}
...
function install() {
  ...
  wrapReviewOpen();
  observeReview();
  installFastReviewSave();
  wrapBuildSessionCard();
  wrapArchiveRender();
  wrapRecentAndPicker();
  observeDecoratedAreas();
  refreshDecorations();
  setTimeout(() => {
    ensureSubtitleField(); wrapPlannerData(); wrapPreviewAndDock(); wrapReviewOpen(); observeReview(); wrapBuildSessionCard(); wrapArchiveRender(); wrapRecentAndPicker(); observeDecoratedAreas(); refreshDecorations();
  }, 250);
  setTimeout(() => {
    ensureSubtitleField(); wrapPlannerData(); wrapReviewOpen(); observeDecoratedAreas(); refreshDecorations();
  }, 900);
